define(["exports", "../core/pubsubhub"], function (_exports, _pubsubhub) {
  ...
  const name = "logius/seo";
  _exports.name = name;

  function run(conf, doc, cb) {
    var trLatestUri = conf.shortName ? "TODO" + conf.pubDomain + "/" + conf.specStatus.substr(3).toLowerCase() + "-" + conf.shortName + "/" : null;
      ...
      switch (conf.canonicalURI.toLowerCase()) {
        case "wv":
          if (conf.edDraftURI) {
            ...
            (0, _pubsubhub.pub)("warn", "Canonical URI staat op WV, " + "maar er is geen edDraftURI geconfigureerd");
        ...
        case "def":
          if (trLatestUri) {
            ...
            (0, _pubsubhub.pub)("warn", "Canonical URI staat op DEF" + "maar er is geen shortName geconfigureerd");
            conf.canonicalURI = null;
          }
