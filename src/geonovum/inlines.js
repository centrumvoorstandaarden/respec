// @ts-check
// Module geonovum/inlines
// Forked of core/inline.js, as suggested by W3C, to fix https://github.com/Geonovum/respec/issues/159
...
export const name = "geonovum/inlines";
  ...
  for (const txt of txts) {
    ...
    while (subtxt.length) {
      ...
      if (matched) {
        // RFC 2119
        if (
          /MOET(?:\s+NIET)?|MOETEN(?:\s+NIET)?|ZOU(?:\s+NIET)?|ZOUDEN(?:\s+NIET)?|MAG|MOGEN|(?:NIET\s+)?VEREIST|(?:NIET\s+)?AANBEVOLEN|OPTIONEEL/.test(
            matched
          )
        ) {
